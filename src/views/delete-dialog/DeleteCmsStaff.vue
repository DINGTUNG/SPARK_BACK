...
async function deleteStaff(staffNoForDelete) {
    try {
        if (staffNoForDelete == null) {
            throw new Error("Staff no. not found!")
        }
        await cmsStaffStore.deleteStaffBackend(staffNoForDelete)
        cmsStaffStore.deleteStaffFromStaffPool(staffNoForDelete)
        window.alert(`刪除成功!剩下 ${cmsStaffStore.staffPool.length} 筆資料`);
    } catch (error) {
        console.error(error);
