...
async function deleteReport(reportNoForDelete) {
  try {
    ...
    window.alert(`刪除成功!剩下 ${reportStore.reportsList.length} 筆資料`);
  } catch (error) {
    console.error(error);
  ...
  <v-row class="row" style="flex: 0 0 0;">
    <v-dialog v-model="dialogDisplay" persistent>
      ...
      <v-card class="delete_dialog" style="border-radius: 50px;">
        <v-card-title class="text-center title" @click="a()">
          確定是否要刪除此報告？
        </v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
          ...
          <v-btn class="delete btn" variant="text" @click="deleteReport(vueProps.reportNoForDelete)">
            刪除
          </v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
