...
const vueProps = defineProps({
  thanksLetterNoForDelete: Number
})
...
async function deleteThanksLetter(thanksLetterNoForDelete) {
  try {
    ...
    await thanksLetterStore.deleteThanksLetterBackend(thanksLetterNoForDelete)
    thanksLetterStore.deleteThanksLetterFromThanksLetterPool(thanksLetterNoForDelete)
    window.alert(`刪除成功!剩下 ${thanksLetterStore.thanksLetterPool.length} 筆資料`);
  } catch (error) {
    console.error(error);
