...
const vueProps = defineProps({
  reportNoForUpdate: Number,
  reportClassForUpdate: String,
  reportYearForUpdate: Number,
  reportTitleForUpdate: String,
  reportFileForUpdate: String,
});
...
function showDialog() {
  ...
  reportForUpdate.reportNo = vueProps.reportNoForUpdate
  reportForUpdate.reportClass = vueProps.reportClassForUpdate
  reportForUpdate.reportYear = vueProps.reportYearForUpdate
  reportForUpdate.reportTitle = vueProps.reportTitleForUpdate
  reportForUpdate.reportFile['name'] = vueProps.reportFileForUpdate
}

async function updateReport() {
  try {
    if (reportForUpdate.reportNo == null) {
      throw new Error("report not found!")
    }
    await reportStore.updateReportBackend(reportForUpdate)
    reportStore.updateReportFileFromReportList(reportForUpdate)
    window.alert(`編輯成功!`);
  } catch (error) {
    console.error(error);
  ...
  <v-row justify="end">
    <v-dialog v-model="dialogDisplay" persistent width="50%">
      ...
      <v-card>
        ...
        <v-card-text>
          <form id="reportFrom" method="POST" action="http://localhost/SPARK_BACK/php/results/report/update_report.php"
            @submit.prevent="updateReport">
            <div class="form_item">
              <div class="name"><span>報告分類</span></div>
              <input type="text" id="title" name="report_class" v-model="reportForUpdate.reportClass">
            </div>
            <div class="form_item">
              <div class="name"><span>年度</span></div>
              <input type="text" id="date" name="report_year" v-model="reportForUpdate.reportYear">
            </div>
            <div class="form_item">
              <div class="name"><span>標題</span></div>
              <input type="text" id="date" name="report_title" v-model="reportForUpdate.reportTitle">
            </div>
            <div class="imgblock form_item">
              <div class="name"><span>報告</span></div>
              <v-file-input id="photo1" prepend-icon="none" name="report_file_path"
