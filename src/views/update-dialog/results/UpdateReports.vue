...
function showDialog() {
  ...
  reportsForUpdate.reportsFile['name'] = vueProps.reportsFileForUpdate
}

async function updateReports(reportsNoForUpdate, reportsClass, reportsYear, reportsTitle, reportsFile) {
  try {
    if (reportsNoForUpdate == null) {
      throw new Error("reports no. not found!")
    }
    await reportStore.updateReportBackend(reportsNoForUpdate, reportsClass, reportsYear, reportsTitle, reportsFile)
    reportStore.updateReportFileFromReportsList(reportsNoForUpdate, reportsClass, reportsYear, reportsTitle, reportsFile)
    window.alert(`編輯成功!`);
  } catch (error) {
    console.error(error);
  ...
  <v-row justify="end">
    <v-dialog v-model="dialogDisplay" persistent width="50%">
      ...
      <v-card>
        ...
        <v-card-text>
          <form id="reportFrom" method="post" action="http://localhost/SPARK_BACK/php/results/reports/update_reports.php"
            @submit.prevent="updateReports(vueProps.reportsNoForUpdate, reportsForUpdate.reportsClass, reportsForUpdate.reportsYear, reportsForUpdate.reportsTitle, reportsForUpdate.reportsFile)">
            <div class="form_item">
            ...
            <div class="imgblock form_item">
              ...
              <v-file-input id="file" prepend-icon="none" name="reports_file_path" accept=".pdf"
                v-model="reportsForUpdate.reportsFile">
                <template v-slot:prepend-inner>
                  <label for="file">修改報告</label>
                </template>
              </v-file-input>
            </div>
            <v-card-actions>
