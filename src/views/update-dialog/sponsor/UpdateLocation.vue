  ...
  <v-row class="row" style="flex: 0;">
    <v-dialog v-model="dialog" persistent width="50%">
      ...
      <v-card>
        ...
        <v-card-text>
          <form action="http://localhost/SPARK_BACK/php/sponsor/sponsor-location/update_sponsor_location.php" method="post"  @submit.prevent="updateLocation(vueProps.locationNoForUpdate,locationName)">
            <label for="local">編輯地區內容
              <input type="text" name="local" v-model="locationName">
            </label>
          </form>
        </v-card-text>
        <v-card-actions>
